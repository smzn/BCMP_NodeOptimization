...
class BCMP_MVA_Computation:
    ...
    def getMVA(self):
        for k_index in range(self.K_total+1):
            ...
            if self.rank == 0:
                for i in range(1, self.size):
                    ...
                    l_rank = self.comm.recv(source=i, tag=14)
                    #print('receive : {0}, {1}'.format(i, k_combi_list_div_rank))
                    
                    #リストの結合
                    for j in k_combi_list_div_rank:
                        #k_state = self.getState(k_combi_list_div_rank[j]) #kの状態を10進数に変換
                        k_state = self.getState(j) #kの状態を10進数に変換
                        for n in range(self.N): #Tの更新
                            for r in range(self.R):
                                self.T[n, r, k_state] = T_rank[n, r, k_state]
                        for r in range(self.R): #Lambdaの更新
                            self.lmd[r,k_state] = lmd_rank[r,k_state]
                        for n in range(self.N):#Lの更新
                            for r in range(self.R):
                                self.L[n,r,k_state] = l_rank[n,r,k_state]
                ...
                self.comm.send(self.T, dest=0, tag=12)
                self.comm.send(self.lmd, dest=0, tag=13)
                self.comm.send(self.L, dest=0, tag=14)
            self.comm.barrier() #プロセス同期
        ...
        last = self.getState(self.K) #combi_listの最終値が最終結果の人数
